...
{
    [CustomEditor(typeof(IllMizukiOptimizer))]
    [AddComponentMenu("")]
    internal class IllMizukiOptimizerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUI.enabled = true;
            {
                ...
                if (BreastSizeFlg1.boolValue != MizukiOptimizer.BreastSizeFlg1)
                {
                    BreastSizeFlg2.boolValue = false;
                    BreastSizeFlg3.boolValue = false;
                }
                else if (BreastSizeFlg2.boolValue != MizukiOptimizer.BreastSizeFlg2)
                {
                    BreastSizeFlg1.boolValue = false;
                    BreastSizeFlg3.boolValue = false;
                }
                else if (BreastSizeFlg3.boolValue != MizukiOptimizer.BreastSizeFlg3)
                {
                    BreastSizeFlg1.boolValue = false;
                    BreastSizeFlg2.boolValue = false;
                }
            ...
            {
                var MizukiOptimizer = (IllMizukiOptimizer)target;
                if (HairFlg10.boolValue != MizukiOptimizer.HairFlg1)
                    HairFlg20.boolValue = false;
                else if (HairFlg20.boolValue != MizukiOptimizer.HairFlg2)
            ...
            if (questArea)
            {
                var mizukiOptimizer = (IllMizukiOptimizer)target;
#if AVATAR_OPTIMIZER_FOUND
                if (mizukiOptimizer.transform.root.GetComponent<TraceAndOptimize>() == null)
                ...
                {
                    if (!HairFlg60.boolValue)
                        if (Head_002.boolValue != mizukiOptimizer.Head_002)
                            Front_hair2_root.boolValue = false;
                        else if (Front_hair2_root.boolValue != mizukiOptimizer.Front_hair2_root)
                            Head_002.boolValue = false;
                }
            ...
            {
                IllMizukiOptimizer script = (IllMizukiOptimizer)target;
        ...
        // Validate メソッドで対象が VRC アバターのルートであり、さらにアタッチされている Animator の avatar が "mizukiAvatar" であることをチェック
        [MenuItem("GameObject/illusive_tools/Create IllMizukiOptimizer Object", true)]
        private static bool ValidateCreateIllMizukiOptimizerObject(MenuCommand menuCommand)
        {
            ...
            if (
                ...
                return false;
            // Animator コンポーネントが存在し、その avatar プロパティの名前が "mizukiAvatar" であるかをチェック
            Animator animator = contextGO.GetComponent<Animator>();
            if (animator == null)
                return false;
            if (animator.avatar == null)
                return false;
            if (animator.avatar.name != "mizukiAvatar")
                return false;
            return true;
        }

        // 対象が条件を満たす場合のみ、メニュー項目が有効となる
        [MenuItem("GameObject/illusive_tools/Create IllMizukiOptimizer Object", false, 10)]
        private static void CreateIllMizukiOptimizerObject(MenuCommand menuCommand)
        {
            // 新しい GameObject を作成し、IllMizukiOptimizer コンポーネントを追加
            GameObject go = new GameObject("IllMizukiOptimizer");
            go.AddComponent<IllMizukiOptimizer>();
